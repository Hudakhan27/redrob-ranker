...
def validate_submission(csv_path: str):
    ...
    if len(data_rows) != EXPECTED_DATA_ROWS:
        errors.append(
            ...
            f'data rows; found {len(data_rows)}.'
        )
    ...
    for i, cells in enumerate(data_rows):
        ...
        if rank is not None and cid:
            by_rank.append((rank, float(score_s), cid))
    ...
    for i in range(len(by_rank) - 1):
        ...
        if s1 == s2 and c1 > c2:
            errors.append(
                f'Equal scores at ranks {r1} and {r2}: tie-break requires candidate_id ascending ({c1} > {c2}).'
            )
